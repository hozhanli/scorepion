...
| `text.tertiary`  | `#475569` | Inactive tab labels, micro meta (upgraded from `#94A3B8` for AAA compliance) |
...
Font family: **Inter** (`Inter_400Regular`, `Inter_500Medium`, `Inter_600SemiBold`, `Inter_700Bold`). All sizes, weights, and tracking live in `type` tokens from `constants/colors.ts`.

| Role    | Token          | Size | Weight | Tracking | Use                                     |
| ------- | -------------- | ---- | ------ | -------- | --------------------------------------- |
| Display | `type.display` | 40   | 700    | -0.8     | Rank hero numbers                       |
| H1      | `type.h1`      | 28   | 700    | 0        | Screen titles (`ScreenHeader`)          |
| H2      | `type.h2`      | 22   | 700    | 0        | Section titles, card titles             |
| H3      | `type.h3`      | 18   | 600    | 0        | List item titles                        |
| Body    | `type.body`    | 15   | 400    | 0        | Primary copy                            |
| Caption | `type.caption` | 13   | 500    | 0        | Metadata, subtitles                     |
