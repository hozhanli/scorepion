...
import {
  Prediction, UserProfile, Group, DailyPackState, DailyPick,
  getPredictions, savePrediction as storePrediction,
  getUserProfile, saveUserProfile,
  getGroups, saveGroups,
  getOnboardingDone, setOnboardingDone as markOnboardingDone,
  saveFavoriteLeagues, getFavoriteLeagues,
  getIsPremium, setIsPremium as storePremium,
  getDailyPack, saveDailyPack,
} from '@/lib/storage';
...
interface AppContextValue {
  ...
  submitPrediction: (matchId: string, homeScore: number, awayScore: number, boosted?: boolean) => Promise<void>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const { data: apiMatches, isLoading: matchesLoading, refetch: refetchMatches } = useFootballMatches();
  ...
  const dailyPickMatches = useMemo(() => {
    ...
    const matched = pickIds.map(id => matches.find(m => m.id === id)).filter(Boolean) as Match[];
  ...
  const initDailyPack = useCallback(async (favLeagues: string[]) => {
  ...
  const completeOnboarding = useCallback(async (username: string, leagueIds: string[]) => {
  ...
  const submitPrediction = useCallback(async (matchId: string, homeScore: number, awayScore: number, boosted?: boolean) => {
    // Guard against rapid submit on same matchId
    if (pendingMatchIds.current.has(matchId)) {
      console.warn('AppContext:submitPrediction guard', { matchId, message: 'submit already in-flight' });
      return;
    }
    ...
    try {
      ...
      try {
        const res = await apiRequest('POST', '/api/predictions', { matchId, homeScore, awayScore });
        const dbPred = await res.json();
        if (dbPred && dbPred.id) {
          setPredictions(prev => ({ ...prev, [matchId]: { ...pred, ...dbPred } }));
        }
      } catch (err) {
        console.error('AppContext:submitPrediction server sync', { matchId, err });
        setLastError({
          message: 'Failed to sync prediction to server',
          timestamp: Date.now(),
        });
      }
      ...
      if (dailyPackRef.current) {
        ...
        if (pickIndex >= 0) {
          ...
          updatedPicks[pickIndex] = { ...updatedPicks[pickIndex], completed: true, boosted: boosted || false };
      ...
      try {
        ...
        console.error('AppContext:submitPrediction daily-pack complete', { matchId, err });
        setLastError({
          message: 'Failed to complete daily pack event',
          timestamp: Date.now(),
        });
      }
      ...
      if (boosted) {
        try {
          ...
          console.error('AppContext:submitPrediction boost', { matchId, err });
          setLastError({
            message: 'Failed to record boost event',
            timestamp: Date.now(),
          });
        }
      }
  ...
  const value = useMemo(() => ({
    ...
    toggleBoostPick,
  }), [matches, predictions, profile, groups, isLoading, onboardingDone, favoriteLeagues, isPremium, dailyPack, dailyPickMatches, lastError, submitPrediction, updateProfile, joinGroup, leaveGroup, createGroup, completeOnboarding, setFavLeagues, upgradeToPremium, refreshDataWithApi, toggleBoostPick]);
