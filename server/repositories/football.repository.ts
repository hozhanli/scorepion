...
import {
    footballLeagues, footballTeams, footballFixtures,
    footballStandings, footballTopScorers, footballTopAssists,
    footballTopYellowCards, footballTopRedCards, footballInjuries,
    footballTransfers, syncLog,
} from "@shared/schema";
...
export async function getFixtures(filters: FixtureFilters = {}) {
  ...
  `, params);
    ...
    return result.rows.map((r: any) => ({
        ...
        league: {
            id: r.l_id, name: r.l_name, country: r.l_country,
            flag: r.l_flag ?? "", color: r.l_color, icon: r.l_icon, logo: r.l_logo ?? "",
        },
        homeTeam: { id: String(r.home_api_id), name: r.home_name, shortName: r.home_short, color: r.home_color, logo: r.home_logo ?? "" },
        awayTeam: { id: String(r.away_api_id), name: r.away_name, shortName: r.away_short, color: r.away_color, logo: r.away_logo ?? "" },
...
export async function getStandings(leagueId: string) {
    const rows = await db.select({ standing: footballStandings, team: footballTeams })
    ...
    return rows.map(r => ({
        ...
        team: {
            ...
            name: r.team.name, shortName: r.team.shortName,
            color: r.team.color, logo: r.team.logo ?? "",
        },
        played: r.standing.played, won: r.standing.won,
        drawn: r.standing.drawn, lost: r.standing.lost,
        goalsFor: r.standing.goalsFor, goalsAgainst: r.standing.goalsAgainst,
        goalDifference: r.standing.goalDifference, points: r.standing.points,
        form: (r.standing.form ?? "").split("").filter((c) => ["W", "D", "L"].includes(c)).slice(-5),
        group: r.standing.group,
    }));
}

export async function getTopScorers(leagueId: string) {
    const rows = await db.select({ scorer: footballTopScorers, team: footballTeams })
    ...
    return rows.map((r, idx) => ({
        ...
        team: { id: String(r.team.apiFootballId), name: r.team.name, shortName: r.team.shortName, color: r.team.color, logo: r.team.logo ?? "" },
        goals: r.scorer.goals, assists: r.scorer.assists, matches: r.scorer.matches,
    }));
}

export async function getTopAssists(leagueId: string) {
    const rows = await db.select({ player: footballTopAssists, team: footballTeams })
    ...
    return rows.map((r, idx) => ({
        ...
        team: { id: String(r.team.apiFootballId), name: r.team.name, shortName: r.team.shortName, color: r.team.color, logo: r.team.logo ?? "" },
        assists: r.player.assists, goals: r.player.goals, matches: r.player.matches,
    }));
}

export async function getTopYellowCards(leagueId: string) {
    const rows = await db.select({ player: footballTopYellowCards, team: footballTeams })
    ...
    return rows.map((r, idx) => ({
        ...
        playerPhoto: r.player.playerPhoto ?? "",
        team: { id: String(r.team.apiFootballId), name: r.team.name, shortName: r.team.shortName, color: r.team.color, logo: r.team.logo ?? "" },
        yellowCards: r.player.yellowCards, matches: r.player.matches,
    }));
}

export async function getTopRedCards(leagueId: string) {
    const rows = await db.select({ player: footballTopRedCards, team: footballTeams })
    ...
    return rows.map((r, idx) => ({
        ...
        playerPhoto: r.player.playerPhoto ?? "",
        team: { id: String(r.team.apiFootballId), name: r.team.name, shortName: r.team.shortName, color: r.team.color, logo: r.team.logo ?? "" },
        redCards: r.player.redCards, matches: r.player.matches,
    }));
}

export async function getInjuries(leagueId: string) {
    const rows = await db.select({ injury: footballInjuries, team: footballTeams })
...
export async function getTransfers(leagueId: string) {
    const rows = await db.select().from(footballTransfers)
...
export async function getFixtureEvents(fixtureApiId: number) {
  ...
  return result.rows.map((r: any) => ({
    ...
    team:       { id: r.team_id, name: r.team_name ?? "", logo: r.team_logo ?? "", color: r.team_color ?? "#333" },
...
export async function getFixtureLineups(fixtureApiId: number) {
  ...
  for (const r of result.rows) {
    ...
    const player = { id: r.player_id, name: r.player_name, number: r.player_number, pos: r.player_pos, grid: r.grid };
...
export async function getH2H(team1ApiId: number, team2ApiId: number) {
  ...
  return result.rows.map((r: any) => ({
    date:     r.kickoff,
    homeTeam: { id: String(r.home_team_id), name: r.home_name, shortName: r.home_short, color: r.home_color, logo: r.home_logo ?? "" },
    awayTeam: { id: String(r.away_team_id), name: r.away_name, shortName: r.away_short, color: r.away_color, logo: r.away_logo ?? "" },
    homeScore: r.home_score,
    awayScore: r.away_score,
    competition: r.league_id ?? "",
  }));
}
...
export async function getSyncStatus(getRequestCount: () => number, getRemainingRequests: () => number) {
  const result = await pool.query(`
