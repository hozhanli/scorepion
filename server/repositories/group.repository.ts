...
export async function createGroup(name: string, code: string, isPublic: boolean, leagueIds: string[], createdBy: string): Promise<Group> {
...
const MEMBER_COLORS = [
    ...
    '#14B8A6', '#6366F1', '#D97706', '#059669', '#DC2626',
    '#7C3AED', '#2563EB', '#DB2777', '#0891B2', '#CA8A04',
];
...
export async function getGroupPredictions(groupId: string): Promise<GroupMatchPredictions[]> {
    ...
    const userIds = memberships.map(m => m.userId);
    const memberUsers = await db.select().from(users).where(inArray(users.id, userIds));
    const userMap = new Map(memberUsers.map((u, idx) => [u.id, {
        username: u.username,
        avatar: u.avatar || u.username.substring(0, 2).toUpperCase(),
        color: MEMBER_COLORS[idx % MEMBER_COLORS.length],
    }]));

    const allPreds = await db.select()
        .from(predictions)
...
export interface GroupActivity {
    id: string;
    type: 'prediction' | 'exact_score' | 'points_earned' | 'streak' | 'joined' | 'boost_pick' | 'achievement' | 'rank_change' | 'weekly_winner';
    username: string;
    avatar: string;
    color: string;
    message: string;
...
export async function getGroupActivity(groupId: string): Promise<GroupActivity[]> {
    ...
    try {
        ...
        if (activityRows.rows.length > 0) {
            return activityRows.rows.map((row: any, idx: number) => {
                const user = { username: row.username, avatar: row.avatar || row.username.substring(0, 2).toUpperCase(), color: MEMBER_COLORS[idx % MEMBER_COLORS.length] };
                const ts = Number(row.created_at);
                const timeAgo = getRelativeTime(ts, now);
                const meta = typeof row.metadata === 'string' ? JSON.parse(row.metadata) : (row.metadata || {});

                const typeMap: Record<string, { message: string; detail?: string; icon: string; iconColor: string }> = {
                    exact_score:   { message: `nailed the exact score! ${meta.predicted || ''}`, detail: `+${row.points} pts${meta.boosted ? ' 🚀' : ''}`, icon: 'locate', iconColor: '#FFD700' },
                    points_earned: { message: `earned points`, detail: `+${row.points} pts`, icon: 'checkmark-circle', iconColor: '#00C853' },
                    streak:        { message: `is on a ${meta.streak}-day streak! 🔥`, detail: `${meta.streak} days`, icon: 'flame', iconColor: '#FF8C00' },
                    boost_pick:    { message: `used a 2x boost pick`, detail: `+${row.points} pts`, icon: 'flash', iconColor: '#FFD700' },
                    achievement:   { message: `unlocked: ${meta.title || 'Achievement'}`, detail: '', icon: 'ribbon', iconColor: '#E040FB' },
                    rank_change:   { message: `moved to rank #${meta.newRank}`, detail: `↑ from #${meta.oldRank}`, icon: 'trending-up', iconColor: '#00C853' },
                    weekly_winner: { message: `won the weekly leaderboard! 🏆`, detail: `#${meta.rank} — ${meta.points} pts`, icon: 'trophy', iconColor: '#FFD700' },
                    prediction:    { message: `made a prediction`, detail: '', icon: 'create', iconColor: '#3B82F6' },
                    joined:        { message: 'joined the group', detail: '', icon: 'person-add', iconColor: '#3B82F6' },
                };
        ...
        console.error('[GroupRepo] group_activity table not yet available, falling back:', (err as any).message);
    }
  ...
  `, [userIds]);
    ...
    const joinedMembers = memberships
