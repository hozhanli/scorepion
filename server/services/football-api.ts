...
const DAILY_LIMIT = parseInt(
  process.env.FOOTBALL_API_DAILY_LIMIT ?? (API_PLAN === "free" ? "100" : "7500"),
  10,
);
const SAFETY_BUFFER = API_PLAN === "free" ? 10 : 200; // conservative buffer for free plan

export function getApiPlan() {
  return API_PLAN;
}
...
// Season values here are FALLBACKS only. On server startup `refreshLeagueSeasons()`
// calls /leagues?id={apiId}&current=true for every entry and overwrites these
// numbers with whatever API-Football reports as the currently active season.
// The fallbacks kick in only when API-Football has no active season for a league
// (e.g. UEFA Nations League between biennial cycles) or the resolver call fails.
//
// Convention: year = starting year of the season in API-Football's terms.
//   2025 = 2025-26 European domestic + UEFA club season
//   2026 = 2026 calendar year (MLS, FIFA World Cup)
//   2024 = 2024-25 Nations League (most recent real edition)
export const LEAGUE_MAP: Record<string, LeagueConfig> = {
  ...
  // Other top domestic leagues (2025-26 season, except MLS which runs on calendar year)
  tsl: {
    apiId: 203,
    season: 2025,
    color: "#E30A17",
  ...
  // UEFA club competitions (2025-26 season)
  ucl: {
    apiId: 2,
    season: 2025,
    color: "#1A237E",
...
export function getActiveLeagues(): Record<string, LeagueConfig> {
  ...
  return Object.fromEntries(Object.entries(LEAGUE_MAP).filter(([id]) => activeIds.includes(id)));
}
...
async function apiGet<T = any>(
  ...
  if (!canMakeRequest()) {
    console.warn(
      `[API-Football] Quota exhausted (${dailyCount}/${DAILY_LIMIT}). Skipping ${endpoint}`,
    );
    return null;
  }
  if (!API_KEY) {
...
export const getFixtureById = (id: number) => apiGet("/fixtures", { id });

export const getFixturesByDate = (date: string) => apiGet("/fixtures", { date });

export const getLiveFixtures = () => apiGet("/fixtures", { live: "all" });
...
export const getTransfers = (team: number) => apiGet("/transfers", { team });

export const getTeamStatistics = (team: number, league: number, season: number) =>
  apiGet("/teams/statistics", { team, league, season });

export const getPredictions = (fixtureId: number) => apiGet("/predictions", { fixture: fixtureId });

// ── Active-season resolution ─────────────────────────────────────────────────
//
// API-Football's /leagues endpoint returns a `seasons` array per league, each
// entry flagged `current: true/false`. Using this beats hardcoding because:
//   - When a season rolls over (e.g. 2025-26 → 2026-27), no code deploy needed
//   - MLS (calendar year), tournaments (one-off), and cup ties with different
//     calendars are each handled correctly by API-Football's own bookkeeping
//   - Off-cycle leagues (e.g. UEFA Nations League between editions) naturally
//     return no current season — we fall back to the LEAGUE_MAP hardcode
...
/**
 * Fetches the currently-active season for a single league from API-Football.
 * Returns null if there is no active season (off-cycle tournament) or the
 * call fails for any reason (quota, network, suspended account, etc.).
 */
export async function resolveCurrentSeason(apiId: number): Promise<number | null> {
  const data = await apiGet<any>("/leagues", { id: apiId, current: "true" });
  const league = Array.isArray(data?.response) ? data.response[0] : null;
  if (!league?.seasons) return null;
  const active = (league.seasons as ApiLeagueSeason[]).find((s) => s.current === true);
  return active?.year ?? null;
}

/**
 * Refreshes LEAGUE_MAP in-place with the currently active season for every
 * configured league. Leagues with no active season (e.g. UEFA Nations League
 * between cycles) keep their hardcoded fallback value.
 *
 * Safe to call on startup and on a daily cron. Spends one API call per league
 * (~15 calls total) which is negligible on any paid plan.
 */
export async function refreshLeagueSeasons(): Promise<{
  updated: number;
  unchanged: number;
  skipped: number;
}> {
  let updated = 0,
    unchanged = 0,
    skipped = 0;
  for (const [localId, cfg] of Object.entries(LEAGUE_MAP)) {
    try {
      const current = await resolveCurrentSeason(cfg.apiId);
      if (current == null) {
        console.log(`[LeagueMap] ${localId}: no active season reported, keeping ${cfg.season}`);
        skipped++;
        continue;
      }
      if (current !== cfg.season) {
        console.log(`[LeagueMap] ${localId}: season ${cfg.season} → ${current}`);
        LEAGUE_MAP[localId].season = current;
        updated++;
      } else {
        unchanged++;
      }
      ...
      console.warn(
        `[LeagueMap] ${localId}: resolve failed (${err.message}), keeping ${cfg.season}`,
      );
      skipped++;
    }
  }
  console.log(
    `[LeagueMap] Season refresh complete — updated: ${updated}, unchanged: ${unchanged}, skipped: ${skipped}`,
  );
