...
leaderboardRouter.get("/", asyncHandler(async (req: Request, res: Response) => {
    try {
        ...
        if (err.name === "UserError") return res.status(err.status || 400).json({ message: err.message });
        console.error("Error fetching leaderboard:", err);
        return res.status(500).json({ message: "Failed to fetch leaderboard" });
    }
}));

userRouter.get("/stats", requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
