...
// POST /api/sync/enrich-historic — backfill events/lineups/stats for all finished fixtures
syncRouter.post(
  "/enrich-historic",
  requireAdmin,
  asyncHandler(async (req: Request, res: Response) => {
    try {
      const batch = Number(req.query.batch) || 30;
      const enriched = await sync.enrichHistoricFixtures(batch);
      return res.json({ ok: true, enriched });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ message: "Historic enrichment failed" });
    }
  }),
);

// POST /api/sync/team-stats — sync team statistics for all leagues
syncRouter.post(
  "/team-stats",
  requireAdmin,
  asyncHandler(async (_req, res: Response) => {
    try {
      const leagues = Object.keys(api.LEAGUE_MAP);
      let total = 0;
      for (const id of leagues) {
        total += await sync.syncTeamStatsForLeague(id);
      }
      return res.json({ ok: true, total });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ message: "Team stats sync failed" });
    }
  }),
);
