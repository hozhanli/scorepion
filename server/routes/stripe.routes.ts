...
stripeRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      // Stripe service signature was updated to (userId, priceKey, returnUrl) —
      // passing priceId as priceKey and building returnUrl from env for legacy route.
      const returnUrl = `${process.env.APP_URL || ""}/billing/success`;
      const session = await stripeService.createCheckoutSession(user.id, priceId as any, returnUrl);
