...
function getDeploymentDomain() {
  // APP_URL is the canonical public URL (e.g. https://scorepion.example.com)
  if (process.env.APP_URL) {
    return stripProtocol(process.env.APP_URL);
  }

  // EXPO_PUBLIC_DOMAIN can also be used for Expo-specific builds
  if (process.env.EXPO_PUBLIC_DOMAIN) {
    return stripProtocol(process.env.EXPO_PUBLIC_DOMAIN);
  }

  console.error(
    "ERROR: No deployment domain found. Set APP_URL or EXPO_PUBLIC_DOMAIN environment variable.",
  );
  process.exit(1);
}
