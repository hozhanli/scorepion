...
> = {
  ...
  sa: [
    ...
    },
    {
      ...
      photo: "https://media.api-sports.io/football/players/310.png",
      teamId: 505,
      goals: 13,
      assists: 5,
      matches: 28,
    },
    {
      playerId: 308,
      name: "Dusan Vlahovic",
      photo: "https://media.api-sports.io/football/players/308.png",
      teamId: 496,
      goals: 12,
      assists: 3,
      matches: 30,
...
function generateStandings(teams: typeof PL_TEAMS, leagueId: string) {
  ...
  return teams
    .map((team, i) => {
      ...
      const won = Math.max(
        0,
        Math.floor((teams.length - i) * 1.5) + Math.floor(Math.random() * 4) - 2,
      );
...
async function seed() {
  ...
  await pool.query(`
    ...
    VALUES ('seed-system', 'scorepion_system', 'system@seed.scorepion.local', '')
  ...
  for (let i = 0; i < fakeUsers.length; i++) {
    ...
    await pool.query(
      `
      INSERT INTO users (id, username, email, avatar, total_points, weekly_points, monthly_points, correct_predictions, total_predictions, streak, best_streak, \`rank\`, favorite_leagues)
      VALUES (?, ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, '["pl","la"]')
    ...
    `,
      [
        `seed-fake-${i + 1}`,
        u.username,
        `${u.username.toLowerCase()}@seed.scorepion.local`,
        u.points,
        Math.floor(u.points / 4),
  ...
  console.log(
    `   - ~${plFixtures.length + laFixtures.length + saFixtures.length} fixtures (past + upcoming)`,
  );
