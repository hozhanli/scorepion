...
import {
  View, Text, StyleSheet, Platform, TextInput, FlatList,
} from 'react-native';
...
import { LEAGUES as FALLBACK_LEAGUES } from '@/lib/mock-data';

type Step = 'welcome' | 'username' | 'leagues' | 'ready';
  ...
  return (
    ...
    >
      <View style={[styles.leagueIcon, selected && { backgroundColor: accent.primary }]}>
        <Ionicons name={league.icon as keyof typeof Ionicons.glyphMap} size={18} color={selected ? '#FFFFFF' : accent.primary} />
      </View>
      <View style={styles.leagueChipInfo}>
        <Text style={[styles.leagueChipName, { color: textRole.primary }]}>{league.name}</Text>
        <Text style={[styles.leagueChipCountry, { color: textRole.tertiary }]}>{league.country}</Text>
      </View>
      <View style={[styles.checkCircle, { borderColor: border.strong }, selected && { backgroundColor: accent.primary, borderColor: accent.primary }]}>
...
export default function OnboardingScreen() {
  ...
  const leagues = contextLeagues.length > 0 ? contextLeagues : FALLBACK_LEAGUES;
  const { user } = useAuth();
  ...
  const stepIndex = ['welcome', 'username', 'leagues', 'ready'].indexOf(step);
  ...
  return (
    <View style={[styles.container, { backgroundColor: surface[1] }]}>
      <View style={[styles.content, { paddingTop: topPad + 16, paddingBottom: botPad + 16 }]}>
        ...
        {step === 'welcome' && (
          <Animated.View entering={FadeInDown.duration(380).springify().damping(16)} style={styles.stepContent}>
            <Animated.View entering={ZoomIn.duration(420).springify().damping(12)} style={styles.welcomeLogo}>
              <ScorpionCrest size={64} accessibilityLabel="Scorpion logo" />
            </Animated.View>

            <Animated.Text entering={entries.fadeInDown(0)} style={[styles.welcomeTitle, { color: textRole.primary }]}>{t.onboarding.welcome.title}</Animated.Text>
            <Animated.Text entering={entries.fadeInDown(1)} style={[styles.welcomeSubtitle, { color: textRole.secondary }]}>
              {t.onboarding.welcome.subtitle}
            </Animated.Text>

            <Animated.View entering={FadeInUp.delay(200).duration(380).springify().damping(16)} style={styles.featureList}>
              {[
                { icon: 'football-outline', title: t.onboarding.welcome.predictTitle,   body: t.onboarding.welcome.predictBody },
                { icon: 'podium-outline',   title: t.onboarding.welcome.climbTitle,     body: t.onboarding.welcome.climbBody },
                { icon: 'people-outline',   title: t.onboarding.welcome.competeTitle,   body: t.onboarding.welcome.competeBody },
                { icon: 'flame-outline',    title: t.onboarding.welcome.streakTitle,    body: t.onboarding.welcome.streakBody },
              ].map((feat, i) => (
                <Animated.View key={feat.title} entering={FadeInUp.delay(260 + i * 70).duration(320)} style={[styles.featureRow, { backgroundColor: surface[0], borderColor: border.subtle }]}>
                  <View style={styles.featureIcon}>
                    <Ionicons name={feat.icon as keyof typeof Ionicons.glyphMap} size={18} color={accent.primary} />
                  </View>
                  <View style={styles.featureCopy}>
                    <Text style={[styles.featureTitle, { color: textRole.primary }]}>{feat.title}</Text>
                    <Text style={[styles.featureBody, { color: textRole.secondary }]}>{feat.body}</Text>
                  </View>
                </Animated.View>
              ))}
            </Animated.View>
        ...
        {step === 'username' && (
          <Animated.View entering={FadeInDown.duration(380).springify().damping(16)} style={styles.stepContent}>
            ...
            <Animated.Text entering={entries.fadeInDown(0)} style={[styles.stepTitle, { color: textRole.primary }]}>{t.onboarding.username.title}</Animated.Text>
            <Animated.Text entering={entries.fadeInDown(1)} style={[styles.stepDesc, { color: textRole.secondary }]}>
            ...
            {usernameTouched && !validation.usernameValid && (
              <Animated.View entering={FadeInDown.duration(240)} style={[styles.inlineError]}>
                <Text style={[styles.inlineErrorText, { color: accent.alert }]}>{validation.usernameError}</Text>
              </Animated.View>
            )}

            <Animated.View entering={FadeInUp.delay(160).duration(380)} style={styles.avatarPreview}>
              <View style={styles.previewAvatar}>
                <Text style={styles.previewAvatarText}>
                  {username.trim().length >= 2 ? username.trim().substring(0, 2).toUpperCase() : '??'}
                </Text>
              </View>
              <Text style={[styles.previewName, { color: textRole.primary }]}>{username.trim() || 'Your Name'}</Text>
        ...
        {step === 'leagues' && (
          <Animated.View entering={FadeInDown.duration(380).springify().damping(16)} style={styles.stepContent}>
            ...
            <Animated.Text entering={entries.fadeInDown(0)} style={[styles.stepTitle, { color: textRole.primary }]}>{t.onboarding.username.leagues.title}</Animated.Text>
            <Animated.Text entering={entries.fadeInDown(1)} style={[styles.stepDesc, { color: textRole.secondary }]}>
        ...
        {step === 'ready' && (
          <Animated.View entering={FadeInDown.duration(420).springify().damping(14)} style={[styles.stepContent, styles.readyContent]}>
            <Animated.View entering={ZoomIn.delay(150).duration(480).springify().damping(10)} style={styles.readyIcon}>
              <View style={styles.readyIconInner}>
                <Ionicons name="checkmark-done" size={40} color="#FFFFFF" />
              </View>
            </Animated.View>
            <Animated.Text entering={entries.fadeInDown(0)} style={[styles.readyTitle, { color: textRole.primary }]}>{t.onboarding.ready.title}</Animated.Text>
            <Animated.Text entering={entries.fadeInDown(1)} style={[styles.readySubtitle, { color: textRole.secondary }]}>
              {t.onboarding.ready.subtitle.replace('{{username}}', username.trim() || 'Player')}
            </Animated.Text>

            <Animated.View entering={FadeInUp.delay(240).duration(380)} style={[styles.readySummary, { backgroundColor: surface[0], borderColor: border.subtle }]}>
              <View style={styles.readySummaryRow}>
                <Ionicons name="person" size={16} color={textRole.secondary} />
                <Text style={[styles.readySummaryText, { color: textRole.primary }]}>{username.trim() || 'Player'}</Text>
              </View>
        ...
        <Animated.View entering={entries.fadeInUp()} style={styles.bottomActions}>
          ...
          <View style={{ flex: 1 }}>
            <Button
              title={step === 'ready' ? t.onboarding.ready.buttonText : step === 'welcome' ? t.onboarding.cta : t.onboarding.next}
