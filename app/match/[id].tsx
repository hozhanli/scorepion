...
import Colors, {
  accent, radii, type as typeTok,
} from '@/constants/colors';
import {
  GradientHero, PressableScale, TeamLogo, LiveBadge,
  FilterSegmented, Button, EmptyState, useCelebration,
} from '@/components/ui';
...
function FormStrip({ form }: { form?: string }) {
  ...
  return (
    ...
    >
      {results.map((r, i) => {
        const color = r === 'W' ? themeAccent.primary
          : r === 'L' ? themeAccent.alert
          : r === 'D' ? themeTextRole.tertiary
          : 'transparent';
        return (
          <View
            key={i}
            style={[
              formStripStyles.square,
...
function H2HMatchRow({ match, index }: { match: H2HMatch; index: number }) {
  const { surface: themeSurface, border: themeBorder, textRole: themeTextRole, accent: themeAccent } = useTheme();
  const dateStr = formatLocalDate(match.date);
  const homeWin = match.homeScore > match.awayScore;
  const awayWin = match.awayScore > match.homeScore;

  return (
    <View style={[h2hStyles.row, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
        <View style={h2hStyles.rowHeader}>
          <Text style={[h2hStyles.date, { color: themeTextRole.tertiary }]}>{dateStr}</Text>
          <Text style={[h2hStyles.comp, { color: themeTextRole.tertiary }]} numberOfLines={1}>{match.competition}</Text>
        </View>
        <View style={h2hStyles.matchContent}>
          ...
          <View style={h2hStyles.scorePills}>
            <View style={[h2hStyles.scorePill, { backgroundColor: themeSurface[2] }, homeWin && { backgroundColor: themeAccent.primary }]}>
              <Text style={[h2hStyles.scorePillText, { color: themeTextRole.primary }, homeWin && { color: '#fff' }]}>{match.homeScore}</Text>
            </View>
            <Text style={[h2hStyles.scoreDash, { color: themeTextRole.tertiary }]}>–</Text>
            <View style={[h2hStyles.scorePill, { backgroundColor: themeSurface[2] }, awayWin && { backgroundColor: themeAccent.primary }]}>
              <Text style={[h2hStyles.scorePillText, { color: themeTextRole.primary }, awayWin && { color: '#fff' }]}>{match.awayScore}</Text>
            </View>
          </View>
          <Text
            style={[
              h2hStyles.teamText,
              h2hStyles.teamTextAway,
              awayWin && h2hStyles.winnerText,
            ]}
            numberOfLines={1}
          >
...
export default function MatchScreen() {
  ...
  const h2hSummary = useMemo(() => {
    ...
    h2hMatches.forEach(m => {
      ...
      if (m.homeScore === m.awayScore) {
        draws++;
      } else if ((m.homeScore > m.awayScore && isHomeTeamHome) || (m.awayScore > m.homeScore && !isHomeTeamHome)) {
        homeWins++;
      } else {
        awayWins++;
      }
  ...
  const isDailyPick = useMemo(() => dailyPack?.picks.some(p => p.matchId === id), [dailyPack, id]);
  ...
  const handleSubmit = async () => {
    ...
    if (submitInFlight.current) {
      console.warn('MatchScreen:handleSubmit guard', { matchId: match.id, message: 'submit already in-flight' });
      return;
    }
  ...
  const Hero = (
    <View style={styles.heroRow}>
      ...
      <View style={styles.heroTeamSide}>
        <View style={[styles.heroLogoWrap, isLive && styles.heroLogoWrapLive, !isLive && { backgroundColor: themeSurface[2] }]}>
          <TeamLogo
        ...
        <Text
          style={[styles.heroTeamName, isLive && styles.heroTeamNameLive, !isLive && { color: themeTextRole.primary }]}
          numberOfLines={1}
        >
        ...
        {(isLive || isFinished) ? (
          <>
            <View style={styles.heroScoreRow}>
              <Text style={[styles.heroScore, isLive && styles.heroScoreLive, !isLive && { color: themeTextRole.primary }]}>{match.homeScore}</Text>
              <Text style={[styles.heroScoreSep, isLive && styles.heroScoreSepLive, !isLive && { color: themeTextRole.tertiary }]}>–</Text>
              <Text style={[styles.heroScore, isLive && styles.heroScoreLive, !isLive && { color: themeTextRole.primary }]}>{match.awayScore}</Text>
            </View>
            {isLive ? (
          ...
          <>
            ...
            {kickoffCountdown ? (
              <View style={styles.heroCountdownChip}>
                ...
                <Text style={[styles.heroCountdownText, { color: themeAccent.primary }]}>{kickoffCountdown}</Text>
              </View>
            ) : null}
          </>
        )}
  ...
  const HeroMeta = (
    <View style={styles.heroMetaRow}>
      <Text style={[styles.heroMetaText, isLive && styles.heroMetaTextLive, !isLive && { color: themeTextRole.secondary }]} numberOfLines={1}>
        {dateStr}
      </Text>
      {isDailyPick && (
        <>
          <View style={[styles.heroMetaDot, isLive && styles.heroMetaDotLive, !isLive && { backgroundColor: themeBorder.subtle }]} />
          <View style={styles.heroMetaInline}>
            <Ionicons name="today" size={11} color={isLive ? '#fff' : themeAccent.primary} />
            <Text style={[styles.heroMetaStrong, isLive && styles.heroMetaStrongLive, !isLive && { color: themeTextRole.primary }]}>Daily pick</Text>
          </View>
        </>
      )}
      {match.tags && match.tags.length > 0 && (
        <>
          ...
          <View style={styles.heroMetaInline}>
            <Ionicons
              name={
                match.tags[0] === 'Derby' ? 'flash' :
                match.tags[0] === 'Final' ? 'trophy' :
                match.tags[0] === 'Knockout' ? 'shield' :
                match.tags[0] === 'Big Match' ? 'star' :
                match.tags[0] === 'Upset Alert' ? 'warning' : 'trending-up'
              }
              size={11}
              color={isLive ? '#fff' : themeAccent.primary}
            />
            <Text style={[styles.heroMetaStrong, isLive && styles.heroMetaStrongLive, !isLive && { color: themeTextRole.primary }]}>
              {match.tags[0]}
            </Text>
          </View>
        </>
  ...
  return (
    ...
    >
      ...
      <View style={[styles.topNav, { paddingTop: topPad + 6 }]}>
        <PressableScale
          onPress={() => router.back()}
          style={[styles.backBtn, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}
          hitSlop={8}
          haptic="light"
        ...
        <PressableScale
          onPress={() => router.push({ pathname: '/league/[id]', params: { id: match.league.id } })}
          style={[styles.leaguePill, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}
          haptic="light"
        >
          <Ionicons name="football-outline" size={12} color={themeTextRole.secondary} />
          <Text style={[styles.leaguePillText, { color: themeTextRole.secondary }]} numberOfLines={1}>{match.league.name}</Text>
        </PressableScale>
      </View>
      ...
      <Animated.View entering={FadeIn.duration(400)} style={styles.heroWrap}>
        {isLive ? (
          ...
          <View style={[styles.heroCardFlat, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
            {Hero}
            {HeroMeta}
          </View>
        )}
      ...
      {activeSection === 'prediction' && (
        <Animated.View entering={FadeInDown.duration(400)} style={styles.predictionSection}>
          {matchStarted && !existingPrediction ? (
            <View style={[styles.lockedCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              ...
              <Text style={[styles.lockedTitle, { color: themeTextRole.primary }]}>Predictions closed</Text>
            ...
            <>
            <Animated.View entering={ZoomIn.duration(300)} style={[styles.predictCardClean, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              {/* Header */}
              <View style={styles.predictHeaderClean}>
                <Text style={[styles.predictLabelClean, { color: themeTextRole.secondary }]}>{t.match.yourPrediction}</Text>
                <View style={styles.predictCountdownChip}>
                  <Ionicons name="lock-closed" size={11} color={themeAccent.primary} />
                  <Text style={[styles.predictCountdownText, { color: themeAccent.primary }]}>Locked</Text>
                </View>
              </View>
              ...
              <View style={styles.predictScoreRow}>
                <View style={styles.predictTeamCol}>
                  <Text style={[styles.predictTeamLabel, { color: themeTextRole.tertiary }]} numberOfLines={1}>
                    {match.homeTeam.shortName}
                  </Text>
                  <Text style={[styles.predictBigScore, { color: themeTextRole.primary }]}>{homeScore}</Text>
                </View>
                ...
                <View style={styles.predictTeamCol}>
                  ...
                  <Text style={[styles.predictBigScore, { color: themeTextRole.primary }]}>{awayScore}</Text>
                </View>
              </View>
              ...
              <View style={styles.submittedFooter}>
                ...
                {!matchStarted && (
                  ...
                  >
                    ...
                    <Text style={[styles.changeLinkText, { color: themeAccent.primary }]}>{t.match.change}</Text>
                  </Pressable>
                )}
              </View>
            </Animated.View>
            ...
            <>
              <View style={styles.predictCardClean}>
                  ...
                  <View style={styles.predictTeamCol}>
                    ...
                    <View style={styles.predictStepperRow}>
                      ...
                      <PressableScale
                        ...
                        style={[
                          styles.predictStepPlus,
                          homeScore >= 15 && { opacity: 0.35 },
                        ]}
                        hitSlop={12}
                        haptic="light"
                      >
                ...
                <View style={styles.predictOutcomeChip}>
                  ...
                  <Text style={[styles.predictOutcomeHint, { color: themeAccent.primary }]}>+10 pts exact</Text>
                </View>
              </View>
              ...
              {isDailyPick && boostAvailable && !isFinished && (
                <PressableScale
                  ...
                  style={[styles.boostToggle, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }, isBoosted && styles.boostToggleActive]}
                >
                  <View style={[styles.boostIconWrap, isBoosted && styles.boostIconWrapActive]}>
                    <Ionicons name="flash" size={16} color={isBoosted ? '#fff' : themeAccent.reward} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.boostText, { color: themeTextRole.primary }, isBoosted && styles.boostTextActive]}>
                      {isBoosted ? t.match.boostActive : t.match.useBoost}
                    </Text>
                    <Text style={[styles.boostSub, { color: themeTextRole.tertiary }]}>{t.match.boostPerDay}</Text>
                  </View>
                  <View style={[styles.boostSwitch, { backgroundColor: themeSurface[2] }, isBoosted && styles.boostSwitchActive]}>
                    <View style={[styles.boostSwitchDot, isBoosted && styles.boostSwitchDotActive]} />
                  </View>
                </PressableScale>
              )}
          ...
          {socialProof && socialProof.totalPredictions > 0 && !isFinished && (
            <View
              style={[styles.socialProofCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}
            >
              <View style={styles.socialProofHeader}>
                <Ionicons name="people-outline" size={14} color={themeAccent.primary} />
                <Text style={[styles.socialProofTitle, { color: themeTextRole.primary }]}>{t.match.communityPicks}</Text>
                <Text style={[styles.socialProofCount, { color: themeTextRole.tertiary }]}>{socialProof.totalPredictions} predictions</Text>
              </View>
              <View style={styles.socialProofPicks}>
                {socialProof.communityPicks.map((pick: { score: string; percent: number }, i: number) => (
                  <View key={i} style={styles.socialProofRow}>
                    <Text style={[styles.socialProofScore, { color: themeTextRole.primary }]}>{pick.score}</Text>
                    <View style={[styles.socialProofBarTrack, { backgroundColor: themeSurface[2] }]}>
                      <View
                        style={[
                          styles.socialProofBarFill,
                          { width: `${pick.percent}%` },
                        ]}
                      />
                    </View>
                    <Text style={[styles.socialProofPercent, { color: themeTextRole.secondary }]}>{pick.percent}%</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View style={[styles.pointsCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
            <View style={styles.pointsTitleRow}>
              <Ionicons name="trophy-outline" size={15} color={themeAccent.primary} />
              <Text style={[styles.pointsTitle, { color: themeTextRole.primary }]}>{t.match.pointsSystem}</Text>
            </View>
            {[
              ...
              <View key={i} style={styles.pointsRow}>
                <View style={styles.pointsIconWrap}>
                  <Ionicons name={item.icon as keyof typeof Ionicons.glyphMap} size={13} color={themeAccent.primary} />
                </View>
                <Text style={[styles.pointsText, { color: themeTextRole.secondary }]}>{item.text}</Text>
        ...
        <View style={styles.predictionSection}>
          {!fixtureEvents || fixtureEvents.length === 0 ? (
            <View style={[styles.lockedCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              ...
              <Text style={[styles.lockedTitle, { color: themeTextRole.primary }]}>{t.match.noEventsYet}</Text>
              <Text style={[styles.lockedText, { color: themeTextRole.secondary }]}>
            ...
            <View style={[eventsStyles.container, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              {fixtureEvents.map((ev: any, i: number) => {
                const isHome = ev.team.id === match.homeTeam.id || String(ev.team.id) === match.homeTeam.id;
                const iconName: keyof typeof Ionicons.glyphMap =
                  ev.type === 'Goal' ? 'football' :
                  ev.detail?.includes('Yellow') ? 'square' :
                  ev.detail?.includes('Red') ? 'square-sharp' :
                  ev.type === 'subst' ? 'swap-horizontal' : 'flash';
                const iconColor =
                  ev.type === 'Goal' ? themeAccent.primary :
                  ev.detail?.includes('Yellow') ? themeAccent.reward :
                  ev.detail?.includes('Red') ? themeAccent.alert :
                  ev.type === 'subst' ? themeTextRole.secondary : themeTextRole.secondary;
                  ...
                  >
                    <View style={eventsStyles.side}>
                      {isHome && (
                        <View style={eventsStyles.eventContent}>
                          <Text style={[eventsStyles.playerName, { color: themeTextRole.primary }]} numberOfLines={1}>
                            {ev.player?.name ?? ''}
                          </Text>
                          {ev.assist?.name && (
                            <Text style={[eventsStyles.assistName, { color: themeTextRole.tertiary }]}>+ {ev.assist.name}</Text>
                          )}
                        </View>
                      )}
                    </View>
                    ...
                    <View style={eventsStyles.center}>
                      <Text style={[eventsStyles.time, { color: themeTextRole.tertiary }]}>
                        {ev.elapsed}{ev.extraTime ? `+${ev.extraTime}` : ''}'
                      </Text>
        ...
        <View style={styles.predictionSection}>
          {!fixtureLineups || fixtureLineups.length === 0 ? (
            <View style={[styles.lockedCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              ...
              <Text style={[styles.lockedTitle, { color: themeTextRole.primary }]}>{t.match.noLineupsYet}</Text>
              <Text style={[styles.lockedText, { color: themeTextRole.secondary }]}>
                {match.status === 'upcoming'
            ...
            <View style={{ gap: 12 }}>
              {fixtureLineups.map((teamData: any, ti: number) => (
                <View key={ti} style={[lineupsStyles.teamCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
                  <View style={lineupsStyles.teamHeader}>
                    ...
                    <Text style={[lineupsStyles.teamName, { color: themeTextRole.primary }]}>{teamData.team?.name}</Text>
                    <View style={[lineupsStyles.formationBadge, { backgroundColor: themeSurface[2] }]}>
                      <Text style={[lineupsStyles.formationText, { color: themeTextRole.secondary }]}>{teamData.formation}</Text>
                    </View>
                  </View>

                  <Text style={[lineupsStyles.subheader, { color: themeTextRole.tertiary }]}>Starting XI</Text>
                  {(teamData.startXI ?? []).map((p: any, i: number) => (
                    ...
                    >
                      <View style={[lineupsStyles.numBadge, { backgroundColor: themeSurface[2] }]}>
                        <Text style={[lineupsStyles.numText, { color: themeTextRole.primary }]}>{p.number ?? '–'}</Text>
                      </View>
                      <Text style={[lineupsStyles.playerName, { color: themeTextRole.primary }]} numberOfLines={1}>{p.name}</Text>
                      <Text style={[lineupsStyles.pos, { color: themeTextRole.tertiary }]}>{p.pos}</Text>
                    </View>
                  ))}

                  {(teamData.subs ?? []).length > 0 && (
                    <>
                      <Text style={[lineupsStyles.subheader, { marginTop: 12, color: themeTextRole.tertiary }]}>Substitutes</Text>
                      {(teamData.subs ?? []).map((p: any, i: number) => (
                        ...
                        >
                          <View style={[lineupsStyles.numBadge, { backgroundColor: themeSurface[2] }]}>
                            <Text style={[lineupsStyles.numTextMuted, { color: themeTextRole.secondary }]}>{p.number ?? '–'}</Text>
                          </View>
                          <Text style={[lineupsStyles.playerNameMuted, { color: themeTextRole.secondary }]} numberOfLines={1}>{p.name}</Text>
                          <Text style={[lineupsStyles.pos, { color: themeTextRole.tertiary }]}>{p.pos}</Text>
                        </View>
                      ))}
                    </>
      ...
      {activeSection === 'h2h' && (
        <View style={styles.h2hSection}>
          <Text style={[styles.sectionTitle, { color: themeTextRole.primary }]}>Head to head</Text>

          {h2hMatches.length === 0 && total === 0 ? (
            <View style={[styles.lockedCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              <View style={[styles.lockedIconWrap, { backgroundColor: themeSurface[2] }]}>
                <Ionicons name="swap-horizontal-outline" size={22} color={themeTextRole.tertiary} />
              </View>
              <Text style={[styles.lockedTitle, { color: themeTextRole.primary }]}>No previous meetings</Text>
              <Text style={[styles.lockedText, { color: themeTextRole.secondary }]}>These teams haven't played each other recently.</Text>
            </View>
          ) : null}

          {(h2hMatches.length > 0 || total > 0) && (
            <>
              <View style={[h2hStyles.summaryCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
                <View style={h2hStyles.summaryTeam}>
                  <TeamLogo
                    ...
                    size={40}
                  />
                  <Text style={[h2hStyles.summaryCount, { color: themeTextRole.primary }]}>{h2hSummary.homeWins}</Text>
                  <Text style={[h2hStyles.summaryLabel, { color: themeTextRole.tertiary }]}>Wins</Text>
                </View>
                <View style={h2hStyles.summaryMiddle}>
                  <View style={[h2hStyles.drawCircle, { backgroundColor: themeSurface[2] }]}>
                    <Text style={[h2hStyles.summaryDrawCount, { color: themeTextRole.primary }]}>{h2hSummary.draws}</Text>
                  </View>
                  <Text style={[h2hStyles.summaryDrawLabel, { color: themeTextRole.tertiary }]}>Draws</Text>
                </View>
                <View style={h2hStyles.summaryTeam}>
                  <TeamLogo
                    logo={match.awayTeam.logo}
                    ...
                    size={40}
                  />
                  <Text style={[h2hStyles.summaryCount, { color: themeTextRole.primary }]}>{h2hSummary.awayWins}</Text>
                  <Text style={[h2hStyles.summaryLabel, { color: themeTextRole.tertiary }]}>Wins</Text>
                </View>
              </View>

              <View style={[h2hStyles.gaugeContainer, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
                <View style={h2hStyles.gaugeLabels}>
                  <Text style={[h2hStyles.gaugePercent, { color: themeTextRole.primary }]}>{homePercent}%</Text>
                  <Text style={[h2hStyles.gaugePercent, { color: themeTextRole.tertiary }]}>{drawPercent}%</Text>
                  <Text style={[h2hStyles.gaugePercent, { color: themeTextRole.primary }]}>{awayPercent}%</Text>
                </View>
                ...
                <View style={h2hStyles.gaugeLegend}>
                  <View style={h2hStyles.legendItem}>
                    <View style={[h2hStyles.legendDot, { backgroundColor: themeAccent.primary }]} />
                    <Text style={[h2hStyles.legendText, { color: themeTextRole.secondary }]}>{match.homeTeam.shortName}</Text>
                  </View>
                  <View style={h2hStyles.legendItem}>
                    <View style={[h2hStyles.legendDot, { backgroundColor: themeSurface[2] }]} />
                    <Text style={[h2hStyles.legendText, { color: themeTextRole.secondary }]}>Draw</Text>
                  </View>
                  <View style={h2hStyles.legendItem}>
                    <View style={[h2hStyles.legendDot, { backgroundColor: themeTextRole.primary }]} />
                    <Text style={[h2hStyles.legendText, { color: themeTextRole.secondary }]}>{match.awayTeam.shortName}</Text>
                  </View>
                </View>
              </View>
      ...
      {activeSection === 'details' && (
        <View style={styles.detailsSection}>
          <Text style={[styles.sectionTitle, { color: themeTextRole.primary }]}>Match details</Text>

          <View style={[detailStyles.card, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
            {[
              ...
              <React.Fragment key={i}>
                {i > 0 && <View style={[detailStyles.divider, { backgroundColor: themeBorder.subtle }]} />}
                <View style={detailStyles.row}>
                  <View style={detailStyles.iconWrap}>
                    <Ionicons name={item.icon as keyof typeof Ionicons.glyphMap} size={16} color={themeAccent.primary} />
                  </View>
                  <View style={detailStyles.textCol}>
                    <Text style={[detailStyles.label, { color: themeTextRole.tertiary }]}>{item.label}</Text>
                    <Text style={[detailStyles.value, { color: themeTextRole.primary }]}>{item.value}</Text>
                  </View>
                </View>
              </React.Fragment>
            ))}
        ...
        <View style={styles.predictionSection}>
          {!fixtureStats || fixtureStats.length === 0 ? (
            <View style={[styles.lockedCard, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
              ...
              <Text style={[styles.lockedTitle, { color: themeTextRole.primary }]}>{t.match.noStatsYet}</Text>
              <Text style={[styles.lockedText, { color: themeTextRole.secondary }]}>
                {match.status === 'upcoming'
                  ? t.match.statsComingSoon
                  : t.match.statsSyncedAfter}
              </Text>
            </View>
          ) : (() => {
            const home = fixtureStats.find((t: any) => t.team.id === match.homeTeam.id || String(t.team.id) === match.homeTeam.id) ?? fixtureStats[0];
            const away = fixtureStats.find((t: any) => t.team.id !== (home?.team?.id)) ?? fixtureStats[1];
            if (!home || !away) return null;

            const statRows: { label: string; homeVal: number | null; awayVal: number | null; isPct?: boolean }[] = [
              { label: t.match.possession,       homeVal: home.statistics.possession,     awayVal: away.statistics.possession,     isPct: true },
              { label: t.match.shotsOnGoal,    homeVal: home.statistics.shotsOnGoal,    awayVal: away.statistics.shotsOnGoal },
              { label: t.match.totalShots,      homeVal: home.statistics.shotsTotal,     awayVal: away.statistics.shotsTotal },
              { label: t.match.shotsInsideBox, homeVal: home.statistics.shotsInsideBox, awayVal: away.statistics.shotsInsideBox },
              { label: t.match.cornerKicks,     homeVal: home.statistics.cornerKicks,    awayVal: away.statistics.cornerKicks },
              { label: t.match.offsides,         homeVal: home.statistics.offsides,       awayVal: away.statistics.offsides },
              { label: t.match.fouls,            homeVal: home.statistics.fouls,          awayVal: away.statistics.fouls },
              { label: t.match.yellowCards,     homeVal: home.statistics.yellowCards,    awayVal: away.statistics.yellowCards },
              { label: t.match.redCards,        homeVal: home.statistics.redCards,       awayVal: away.statistics.redCards },
              { label: t.match.gkSaves,         homeVal: home.statistics.saves,          awayVal: away.statistics.saves },
              { label: t.match.totalPasses,     homeVal: home.statistics.totalPasses,    awayVal: away.statistics.totalPasses },
              { label: t.match.accuratePasses,  homeVal: home.statistics.accuratePasses, awayVal: away.statistics.accuratePasses },
            ].filter(r => r.homeVal != null || r.awayVal != null);

            return (
              <View style={[matchStatsStyles.container, { backgroundColor: themeSurface[0], borderColor: themeBorder.subtle }]}>
                <View style={[matchStatsStyles.headerRow, { borderBottomColor: themeBorder.subtle }]}>
                  <View style={matchStatsStyles.teamHeaderSide}>
                    <TeamLogo
                      ...
                      size={24}
                    />
                    <Text style={[matchStatsStyles.teamHeaderName, { color: themeTextRole.primary }]} numberOfLines={1}>
                      {match.homeTeam.shortName}
                    </Text>
                  </View>
                  <Text style={[matchStatsStyles.headerLabel, { color: themeTextRole.secondary }]}>Stats</Text>
                  <View style={[matchStatsStyles.teamHeaderSide, matchStatsStyles.teamHeaderRight]}>
                    <Text style={[matchStatsStyles.teamHeaderName, { color: themeTextRole.primary }]} numberOfLines={1}>
                      {match.awayTeam.shortName}
                    </Text>
                    <TeamLogo
                ...
                {statRows.map((row, i) => {
                  ...
                  return (
                    <View key={i} style={[matchStatsStyles.statRow, { borderBottomColor: themeBorder.subtle }]}>
                      <Text
                        style={[
                      ...
                      <View style={matchStatsStyles.barWrap}>
                        <Text style={[matchStatsStyles.statLabel, { color: themeTextRole.secondary }]}>{row.label}</Text>
                        <View style={[matchStatsStyles.barTrack, { backgroundColor: themeSurface[2] }]}>
                          <View
                            style={[
                              matchStatsStyles.barHome,
                              {
                                flex: hPct,
                                backgroundColor: homeWins ? themeAccent.primary : themeTextRole.tertiary,
                              },
                            ]}
                          />
                          <View
                            style={[
                              ...
                              {
                                flex: 1 - hPct,
                                backgroundColor: awayWins ? themeAccent.primary : themeTextRole.tertiary,
                              },
                            ]}
                          />
                        </View>
