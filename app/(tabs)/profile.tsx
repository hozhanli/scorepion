...
import {
  GradientHero, PressableScale, TierBadge, ProgressBar, ScreenHeader, Button,
} from '@/components/ui';
  ...
  return (
    <View style={{ flex: 1 }}>
      <PressableScale style={{ flex: 1 }} haptic="light">
        <View style={[statStyles.card, { backgroundColor: surface[0], borderColor: border.subtle }]}>
          <View style={statStyles.iconWrap}>
            <Ionicons name={icon} size={20} color={accent.primary} />
          </View>
          <StyledText variant="h3" style={statStyles.value}>{value}</StyledText>
          <View style={statStyles.accentStripe} />
          <StyledText variant="caption" role="tertiary" style={statStyles.label}>{label}</StyledText>
        </View>
      </PressableScale>
    </View>
  );
...
// ── Achievement card ─────────────────────────────────────────────────────────
function AchievementCard({ achievement, isUnlocked, progress, index }: {
  achievement: any;
  isUnlocked: boolean;
  progress?: number;
  index: number;
  ...
  return (
    <View style={{ flex: 1, minWidth: '31%' }}>
      <View style={[achStyles.card, { backgroundColor: surface[0], borderColor: border.subtle }, isUnlocked && achStyles.cardUnlocked]}>
        {isUnlocked && (
        ...
        <Text style={[
          achStyles.title,
          { color: isUnlocked ? textRole.primary : textRole.secondary },
        ]} numberOfLines={2}>
        ...
        {isUnlocked && (
          <Text style={achStyles.unlockedLabel}>Unlocked</Text>
        )}
      </View>
    </View>
  );
  ...
  return (
    <View style={sectionStyles.row}>
      <View style={sectionStyles.titleWrap}>
        ...
        {subtitle && <Text style={[sectionStyles.subtitle, { color: textRole.tertiary }]}>{subtitle}</Text>}
      </View>
      {action && onAction && (
...
export default function ProfileScreen() {
  ...
  const SettingsAction = (
    <PressableScale
      ...
      pressedScale={0.92}
    >
      <View style={[styles.headerAction, { backgroundColor: surface[0], borderColor: border.subtle }]}>
    ...
    >
      ...
      {isLoading ? (
        ...
        <View style={styles.section}>
          ...
          >
            <View style={styles.heroTop}>
              ...
              <View style={styles.heroTextCol}>
                <TierBadge tier={getLevelTier(levelData.level)} level={levelData.level} size="sm" solid />
            ...
            <View style={styles.heroProgress}>
              <View style={styles.heroProgressHeader}>
                <Text style={styles.heroLevelName}>
                  {levelData.name}
                </Text>
              ...
              <ProgressBar progress={levelData.xpPct} colors={['#FFFFFF', 'rgba(255, 255, 255, 0.9)']} height={6} />
              <Text style={styles.heroNextCopy}>
                {levelData.level < 7
                  ? tt(t.profile.pointsToNext, { points: levelData.pointsToNext, level: levelData.level + 1 })
          ...
          >
            ...
            <Text style={styles.shareTierBtnText}>
              {t.profile.shareMyTier}
            </Text>
          </PressableScale>
        </View>
      )}
      ...
      <View style={styles.section}>
        <SectionHeader
          ...
          subtitle={unlockedCount ? tt(t.profile.unlocked, { count: unlockedCount }) : t.profile.startEarningBadges}
          action={achievements.length > 4 ? t.common.seeAll : undefined}
          onAction={() => router.push('/scoring')}
        />

        {achievements.length === 0 ? (
          <View style={[styles.achEmpty, { backgroundColor: surface[0], borderColor: border.subtle }]}>
            <View style={styles.achEmptyIcon}>
              <Ionicons name="trophy-outline" size={28} color={accent.primary} />
            </View>
            <Text style={[styles.achEmptyTitle, { color: textRole.primary }]}>{t.profile.startEarningBadges}</Text>
            <Text style={[styles.achEmptySub, { color: textRole.tertiary }]}>{t.profile.makePredictions}</Text>
          </View>
        ) : (
      ...
      <View style={styles.section}>
        <SectionHeader
          ...
          subtitle={groups.length > 0 ? tt(t.profile.groupsActive, { count: groups.length }) : undefined}
          action={t.profile.manage}
          onAction={() => router.push('/(tabs)/groups')}
        />
        {groups.length === 0 ? (
          <View style={[styles.groupsEmpty, { backgroundColor: surface[0], borderColor: border.subtle }]}>
          ...
          <View style={styles.groupsGrid}>
            {groups.slice(0, 4).map((g: any) => (
              <PressableScale
                ...
                haptic="light"
              >
                <View style={[styles.groupChip, { backgroundColor: surface[0], borderColor: border.subtle }]}>
                  <Text style={[styles.groupName, { color: textRole.primary }]} numberOfLines={1}>{g.name}</Text>
      ...
      {/* ── 5. Invite friends — secondary emerald hero ── */}
      <View style={styles.section}>
        <PressableScale
          onPress={() => Share.share({ message: t.profile.inviteMessage || 'Join me on Scorepion!' })}
          haptic="medium"
        >
          <View style={[styles.inviteCard, { backgroundColor: surface[0], borderColor: border.subtle }]}>
            <View style={styles.inviteIcon}>
              <Ionicons name="people-outline" size={22} color={accent.primary} />
            </View>
            <View style={styles.inviteText}>
              <Text style={[styles.inviteTitle, { color: textRole.primary }]}>{t.profile.inviteFriends}</Text>
              <Text style={[styles.inviteSub, { color: textRole.tertiary }]}>{t.profile.inviteSubtitle}</Text>
            </View>
