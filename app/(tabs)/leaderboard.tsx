...
import {
  View, Text, FlatList, StyleSheet, Platform, ActivityIndicator,
} from 'react-native';
...
import {
  PressableScale, GradientHero, TierBadge, StatChip, ProgressBar,
  ScreenHeader, FilterSegmented, Button, EmptyState,
} from '@/components/ui';
...
const periodStyles = StyleSheet.create({
  ...
  countdownLabel: { fontSize: 9, fontFamily: 'Inter_600SemiBold', letterSpacing: 0.3, textTransform: 'uppercase' },
...
function Podium({ top3 }: { top3: LBEntry[] }) {
  ...
  const medalTints = top3.length >= 3
  ...
  return (
    <View>
      <View style={[podiumStyles.card, { backgroundColor: surface[0], borderColor: border.subtle }]}>
            ...
            return (
              ...
              >
                ...
                <View style={podiumStyles.avatarWrap}>
                  ...
                  >
                    <Text style={[podiumStyles.avatarText, { fontSize: isChamp ? 24 : 18, color: textRole.primary }]}>
                ...
                <View style={[
                  podiumStyles.base,
                  { height: pillHeights[i], backgroundColor: surface[2], borderColor: surface[0] },
                ]}>
                  <Text style={[podiumStyles.baseRank, { color: textRole.secondary }]}>#{entry.rank}</Text>
                </View>
              </View>
            );
          })}
...
const podiumStyles = StyleSheet.create({
  ...
  name: { fontSize: 12, fontFamily: 'Inter_700Bold', textAlign: 'center', maxWidth: 100, marginTop: 2 },
...
function YourRankHeroCard({
  profile, rank, accuracy, streak, bestStreak, leaderboard,
}: {
  profile: any;
  rank: number;
  accuracy: number;
      ...
      >
        ...
        <View style={youStyles.mainRow}>
          ...
          <View style={youStyles.identityCol}>
            <Text style={youStyles.username} numberOfLines={1}>{profile.username}</Text>
        ...
        <View style={youStyles.progressSection}>
          ...
          <ProgressBar progress={progress} colors={['#FFFFFF', 'rgba(255, 255, 255, 0.9)']} height={6} />
        </View>
...
const youStyles = StyleSheet.create({
  ...
  eyebrow: { fontSize: 11, fontFamily: 'Inter_700Bold', color: 'rgba(255,255,255,0.85)', letterSpacing: 0.6 },
  trendChip: {
  ...
  rankHash: { fontSize: 22, fontFamily: 'Inter_700Bold', color: 'rgba(255,255,255,0.7)', lineHeight: 32, paddingTop: 8 },
  rankNum: { fontSize: 56, fontFamily: 'Inter_700Bold', color: '#fff', letterSpacing: -2.5, lineHeight: 60 },
  identityCol: { flex: 1 },
  username: { fontSize: 17, fontFamily: 'Inter_700Bold', color: '#fff' },
  points: { fontSize: 20, fontFamily: 'Inter_700Bold', color: '#fff', marginTop: 4, letterSpacing: -0.5 },
  ...
  progressLabel: { flex: 1, fontSize: 12, fontFamily: 'Inter_600SemiBold', color: 'rgba(255,255,255,0.92)' },
  ...
  statLabel: { fontSize: 10, fontFamily: 'Inter_500Medium', color: 'rgba(255,255,255,0.7)', letterSpacing: 0.2 },
});
  ...
  return (
    <View style={[chaseStyles.wrap, { backgroundColor: surface[0], borderColor: border.subtle }]}>
      ...
      <View style={chaseStyles.body}>
        ...
        <StyledText variant="caption" role="tertiary" style={chaseStyles.rank}>#{targetRank}</StyledText>
      </View>
      ...
      >
        <View
          style={[
            ...
            isCurrentUser && { ...rowStyles.rowActive, backgroundColor: 'rgba(0, 166, 81, 0.09)', borderColor: accent.primary },
          ]}
        >
          ...
          <View style={rowStyles.rankCol}>
            {isTop3 ? (
              <View style={[
                rowStyles.medal,
                { backgroundColor: [medalColors.gold, medalColors.silver, medalColors.bronze][entry.rank - 1] },
              ]}>
          ...
          <View style={[rowStyles.avatar, { backgroundColor: surface[2] }, isCurrentUser && rowStyles.avatarHighlight]}>
          ...
          <View style={rowStyles.infoCol}>
            <View style={rowStyles.nameRow}>
              <Text
                style={[rowStyles.username, { color: textRole.primary }, isCurrentUser && rowStyles.usernameHighlight]}
                numberOfLines={1}
              >
                {entry.username}
              </Text>
            ...
            <View style={rowStyles.microRow}>
              ...
              {entry.total > 0 && (
                <StatChip icon="locate" value={Math.round((entry.correct / entry.total) * 100)} label="%" color={accent.primary} />
              )}
            </View>
          </View>

          <View style={rowStyles.pointsCol}>
            <Text style={[rowStyles.points, { color: textRole.primary }, isCurrentUser && rowStyles.pointsHighlight]}>
              {entry.points.toLocaleString()}
            </Text>
            <View style={rowStyles.deltaRow}>
              {entry.change > 0 ? (
                <>
                  ...
                  <Text style={[rowStyles.deltaText, { color: accent.primary }]}>+{entry.change}</Text>
                </>
              ) : entry.change < 0 ? (
                <>
...
const sectionStyles = StyleSheet.create({
  ...
  label: { fontSize: 11, fontFamily: 'Inter_700Bold', letterSpacing: 1, textTransform: 'uppercase' },
...
export default function LeaderboardScreen() {
  ...
  const userRank = useMemo(() => {
    ...
    if (!profile) return 0;
    const found = (leaderboard as LBEntry[]).find(e => e.username === profile.username);
    if (found) return found.rank;
    const above = (leaderboard as LBEntry[]).filter(e => e.points > (profile.totalPoints ?? 0)).length;
    return above + 1;
  }, [profile, leaderboard, chaseData]);
  ...
  const chaseTarget = useMemo(() => {
    if (!profile) return null;
    const entries = leaderboard as LBEntry[];
  ...
  const top3 = (leaderboard as LBEntry[]).slice(0, 3);
  const rest = (leaderboard as LBEntry[]).slice(3);
  ...
  const renderRow = useCallback(({ item, index }: { item: LBEntry; index: number }) => (
    <LeaderboardRow
      entry={item}
      index={index}
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <ScreenHeader
              title="Leaderboard"
              subtitle={`${leaderboard.length} players competing`}
              showLogo
            />
            ...
            {isLoading ? (
              ...
              <View style={styles.emptyWrap}>
                <EmptyState
                  ...
                  subtitle="Start predicting to claim the top spot!"
                />
              </View>
            ) : (
              <Podium top3={top3} />
            ...
            {weeklyWinners && weeklyWinners.length > 0 && (
              <>
                ...
                <View style={[styles.winnersCard, { backgroundColor: surface[0], borderColor: border.subtle }]}>
                  {weeklyWinners.slice(0, 3).map((w: any, i: number) => {
                    ...
                    return (
                      <View key={w.id || i} style={styles.winnerRow}>
                        ...
                        <Text style={[styles.winnerName, { color: textRole.primary }]} numberOfLines={1}>{w.username}</Text>
                        <Text style={[styles.winnerPts, { color: textRole.primary }]}>{w.points} pts</Text>
                      </View>
                    );
                  })}
                </View>
...
const styles = StyleSheet.create({
  ...
  winnerMedal: { width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
