...
import {
  View, Text, StyleSheet, Platform, TextInput, ActivityIndicator,
  ...
  return (
    <View style={[styles.container, { backgroundColor: surface[1] }]}>
      ...
      >
        ...
        >
          {/* Logo lockup */}
          <Animated.View entering={ZoomIn.duration(420).springify().damping(12)} style={styles.logoWrap}>
          ...
          <Animated.View entering={FadeInDown.delay(120).duration(380).springify().damping(16)}>
            ...
            <Text style={[styles.tagline, { color: textRole.secondary }]}>Predict. Compete. Climb.</Text>
          </Animated.View>

          {/* Mode segmented control */}
          <Animated.View entering={FadeInUp.delay(200).duration(380).springify().damping(16)} style={[styles.modeRow, { backgroundColor: surface[2] }]}>
            <ModeTab
              label="Sign In"
              active={mode === 'login'}
          ...
          {/* Form */}
          <Animated.View entering={FadeInUp.delay(260).duration(380).springify().damping(16)} style={styles.form}>
            ...
            <View style={[
              ...
              !error && touched.username && validation.usernameValid && { borderColor: accent.primary },
              ...
              />
              {!error && touched.username && validation.usernameValid && (
                <Ionicons name="checkmark-circle" size={20} color={accent.primary} style={{ marginLeft: 8 }} accessibilityLabel="Username is valid" />
              )}
            </View>
            ...
            <View style={[
              ...
              !error && touched.password && validation.passwordValid && { borderColor: accent.primary },
              ...
              />
              {!error && touched.password && validation.passwordValid && (
                <Ionicons name="checkmark-circle" size={20} color={accent.primary} style={{ marginRight: 8 }} accessibilityLabel="Password is valid" />
              )}
              <PressableScale onPress={() => setShowPassword(!showPassword)} style={styles.eyeBtn} haptic="selection" pressedScale={0.9} accessibilityLabel={showPassword ? 'Hide password' : 'Show password'}>
                <Ionicons name={showPassword ? 'eye-off-outline' : 'eye-outline'} size={20} color={textRole.tertiary} />
              </PressableScale>
            </View>

            {touched.password && !validation.passwordValid && (
              <Animated.View entering={FadeInDown.duration(240)} style={[styles.inlineError]}>
                <Text style={[styles.inlineErrorText, { color: accent.alert }]}>{validation.passwordError}</Text>
              </Animated.View>
            )}
            ...
            {mode === 'register' ? (
              <Text style={[styles.hint, { color: textRole.tertiary }]}>Username min. 2 characters · Password min. 4 characters</Text>
            ) : null}
          </Animated.View>
        </ScrollView>
      </KeyboardAvoidingView>
...
function ModeTab({ label, active, onPress, surface, textRole }: { label: string; active: boolean; onPress: () => void; surface: any; textRole: any }) {
  return (
    <PressableScale onPress={onPress} haptic="selection" pressedScale={0.98} style={[styles.modeTab, active && { backgroundColor: surface[0] }]}>
      <Text style={[styles.modeText, { color: active ? textRole.primary : textRole.secondary }]}>{label}</Text>
    </PressableScale>
  );
}
