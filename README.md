...
A football prediction app built with Expo (React Native) + Express + MySQL.
...
macOS uses port 5000 for AirPlay Receiver. This project already avoids it by defaulting to port 5847. If you still hit a conflict, either change `PORT` in `.env` or disable AirPlay Receiver in _System Settings > General > AirPlay & Handoff_.
...
This means the database URL isn't being picked up (MySQL defaults to 3306). Verify `.env` has `DATABASE_URL=mysql://root:root@localhost:13292/scorepion`.
...
├── scripts/
│   ├── start-db.sh       # Docker MySQL management
